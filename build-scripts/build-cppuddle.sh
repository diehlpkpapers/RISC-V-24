...
${CMAKE_COMMAND} -H${DIR_SRC} -B${DIR_BUILD} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_CXX_COMPILER=$CXX -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DCMAKE_INSTALL_PREFIX=${DIR_INSTALL}/cppuddle -DCPPUDDLE_WITH_TESTS=OFF -DCPPUDDLE_WITH_COUNTERS=OFF -DCPPUDDLE_WITH_MULTIGPU_SUPPORT=OFF  -DHPX_DIR=$INSTALL_ROOT/hpx/${LIB_DIR_NAME}64/cmake/HPX/ -DCPPUDDLE_WITH_HPX=ON -DCPPUDDLE_WITH_HPX_MUTEX=ON
