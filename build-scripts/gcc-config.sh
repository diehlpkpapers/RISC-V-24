...
if [[ -d "/etc/opt/cray/release/" ]]; then
  ...
  if [ "${OCT_WITH_KOKKOS}" == "ON" ] && [ "${OCT_WITH_CUDA}" == "ON" ]; then 
    ...
    export OCT_CMAKE_CXX_COMPILER="g++"
    export OCT_CMAKE_CXX_COMPILER_INITIAL="g++"
  fi
fi
...
case $(uname -i) in
    ...
    unknown)
        export CXX=g++
        export CC=gcc
        export CXXFLAGS="-fPIC -march=rv64imafdcv -ffast-math -std=c++20 "
	    export OCT_ARCH_FLAGS="-march=rv64imafdcv"
        export LIBHPX=lib
        ;;
    *)
